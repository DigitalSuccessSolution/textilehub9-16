        ...
        <div className="flex flex-col gap-5 text-left">
          {jobs.map((job, idx) => (
            <motion.div
              ...
              key={job.id}
              className="group flex flex-col lg:flex-row items-start lg:items-center justify-between bg-white transition-all duration-300 hover:shadow-md p-6 md:p-8 rounded-2xl"
              style={{ border: '1.5px solid #EADBC8' }}
            >
              <div className="flex-grow max-w-4xl lg:pr-8">
                <h3 className="text-xl sm:text-2xl font-bold mb-3 leading-tight text-[#2A083E]"
                  style={{ fontFamily: "'Cormorant Garamond', serif" }}>
                  {job.title}
                </h3>

                <div className="flex flex-wrap items-center gap-4 md:gap-6 mb-4">
                  <div className="flex items-center gap-2 text-[13px] font-semibold text-[#786B80]">
                    <MapPin size={14} color="#C5A059" /> {job.location}
                  </div>
                  <div className="flex items-center gap-2 text-[13px] font-semibold text-[#786B80]">
                    <Briefcase size={14} color="#C5A059" /> {job.type} · {job.experience}
                  </div>
                  <div className="flex items-center gap-2 text-[13px] font-semibold text-[#786B80]">
                    <Mail size={14} color="#C5A059" />
                    <a href="mailto:careers@mayuraroyaltextile.com" className="hover:underline text-[#C5A059]">
                      careers@mayuraroyaltextile.com
                    </a>
                  </div>
                </div>

                <p className="text-[13.5px] leading-relaxed mb-6 lg:mb-0 text-[#504257] font-medium">{job.description}</p>
              </div>

              <div className="w-full lg:w-auto shrink-0 border-t lg:border-t-0 lg:border-l pt-5 lg:pt-0 lg:pl-8 border-[#EADBC8]">
                <button
                  className="w-full lg:w-44 py-3.5 text-[11px] font-bold uppercase tracking-widest transition-all duration-200 hover:opacity-95 text-white btn-primary shadow-sm"
                >
                  Apply Now
                </button>
              </div>
